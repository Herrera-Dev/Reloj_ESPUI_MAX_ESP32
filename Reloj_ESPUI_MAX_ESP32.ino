...
#define MOD_RTC 1   // Modulo RTC DS3231
...
void setupESPUI()
{
  ...
  ESPUI.setElementStyle(ESPUI.addControl(Label, "", "Ciudad (London,GB)", None, apiOWM), labelsPer);
...
void leerDHT()
{
  float t, h;
#if MOD_DHT
  t = dht.readTemperature() + (calibTemp);
  h = dht.readHumidity();
#endif
...
void leerCreditoSerpApi()
{
  ...
  Serial.println("===== SerpApi Cuenta =====");
  Serial.printf("Estado: %s\n", statusSA);
  Serial.printf("Restante mensual: %d\n", restSA);
  http.end();
}
...
void leer_serapApi()
{
  ...
  int result = snprintf(url, sizeof(url), "https://serpapi.com/search.json?q=%s&location=%s&hl=en&api_key=%s&json_restrictor=sports_results&no_cache=true",
                        miLlave.equipo, miLlave.location, miLlave.api_key[countSA]);
