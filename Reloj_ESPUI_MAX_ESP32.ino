...
// RTC
  ...
  case ESP_RST_POWERON:
    ...
    P.displayText("RELOJ", PA_CENTER, catalogo[1].velocidad, catalogo[1].pausa * 3000, catalogo[1].efecto, catalogo[1].efecto);
    break;
  }
}
...
void sonarAlarma()
{
  ...
  { // Alarma sonando
    ...
    {
      P.displayReset();
      P.displayText(" ", PA_CENTER, 0, 1, PA_NO_EFFECT, PA_NO_EFFECT);
      mensAlarm = false;
    }

    if (P.displayAnimate())
    {
      P.displayText("<< Alarma >>", PA_CENTER, memMensaVeloc, 0, catalogo[3].efecto, catalogo[3].efecto);
    }

    silenciar = (digitalRead(pulsador) == HIGH) ? true : silenciar; // Silenciar
    if (silenciar)
    {
...
{
  static String dato;
  static textPosition_t posi = PA_CENTER;
      ...
      else
      {
        ...
        case 0: // RELOJ
          ...
          { // inicio
            ...
            P.displayText(dato.c_str(), posi, catalogo[miEfecto].velocidad, 600, catalogo[miEfecto].efecto, PA_NO_EFFECT);
            rInicio = false;
          }
          else
          {
            ...
            {
              dato = String(miHoraV);
              P.displayText(dato.c_str(), posi, 0, 600, PA_NO_EFFECT, PA_NO_EFFECT);
            }
            else
            {
        ...
        case 1: // TEMPERATURA
          ...
          { // fin
            ...
            {
              dato = String(miHoraV);
              P.displayText(dato.c_str(), posi, catalogo[miEfecto].velocidad, 600, PA_NO_EFFECT, catalogo[miEfecto].efecto);
            }
            else
            {
              dato = String(catalogoVista[0].datos);
          ...
          else
          {
            ...
            P.displayText(dato.c_str(), posi, catalogo[miEfecto].velocidad, catalogoVista[modo].tiempo * 1000, catalogo[miEfecto].efecto, catalogo[miEfecto].efecto);
          }
          break;
        ...
        case 3:
          dato = String(miIp);
          P.displayText(dato.c_str(), posi, catalogo[3].velocidad + 15, 0, catalogo[3].efecto, catalogo[3].efecto);
          tModoAnt = millis();
          break;
        }
      }
